...
- Concrete adapters/routes are missing or incomplete:
  ...
  ```text
  free OpenRouter VLM/LLM auto-route wiring from prefix resolver into planner/executor
...
| 5 visual/context enrichment | Configured VLM + typed model-ref resolver slices implemented | Explicit visual/full workflows can plan sample+OCR+describe+capture when current provider-alias routes are available, extract PNG frame artifacts, write `visual_records.json`, and render OCR/description/frame refs. Prefix resolver can infer `openrouter:`, `local:`, `hf:`, `auto`, and `none`, including free OpenRouter text+image model selection from registry metadata, but it is not yet wired into visual route discovery/execution. Transcript-anchored/evidence-guided sampling remains missing. |
